...
func (s *Store) SetBaseline(id string, baselineLog float64, hasBaseline bool, updatedAt int64) error {
	res, err := s.db.Exec(
		`UPDATE speakers SET baseline_log=?, has_baseline=?, created_at=? WHERE id=?`,
		baselineLog, boolToInt(hasBaseline), updatedAt, id)
	if err != nil {
		return err
	}
